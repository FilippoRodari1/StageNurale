...
import { RootState, fetchFatturaAcquisti, useAppDispatch } from "../../../store";
...
import FatturaAcquistoModal from "./fatturaAcquistoModal";
import FatturaAcquistoTable from "./fatturaAcquistoTable";
...
const FatturaAcquisto = () => {
    ...
    const [resourceToDelete, setResourceToDelete] = useState<PurchaseInvoice | null>(null);
    ...
    const purchaseInvoice = useSelector((state: RootState) => state.purchaseInvoice.data);

    useEffect(() => {
        dispatch(fetchFatturaAcquisti());
    }, [dispatch]);

    const handleSave = async (data: PurchaseInvoice) => {
        try {
            if (editingId !== null) {
                await updateData(editingId, { ...data, id: editingId });
            } else {
                await dispatch(createFattureAcquisti(data));
            }
            handleModalClose();
        } catch (error: any) {
    ...
    const handleDeleteConfirmation = (resource: PurchaseInvoice) => {
    ...
    const deleteData = async (id: number) => {
        try {
             await dispatch(deleteFattureAcquisti(id));
    ...
    const updateData = async (id: number, updatedResource: PurchaseInvoice) => {
        try {
            const response = await fetch(`${URL_FATTURADIACQUISTO}/${id}`, {
                method: 'PUT',
    ...
    const handleModalOpen = (id?: number) => {
        if (id) {
            const resource = purchaseInvoice.find((resource: PurchaseInvoice) => resource.id === id);
            if (resource) {
    ...
    return (
        <div className={`h-full w-full flex-col ${isDarkMode ? 'dark-mode' : ''}`}>
            ...
            <Navbar name="Fatture di Acquisto" />
                    ...
                    {isDeleteModalOpen && resourceToDelete && (
                        <ModalDelete show={isDeleteModalOpen} onClick={() => setIsDeleteModalOpen(false)} className="bg-white rounded-xl p-8 max-w-md w-full flex flex-col items-center justify-center">
                            <div className="bg-white rounded-lg p-6 max-w-md w-full flex flex-col items-center justify-center">
                                <p className="font-lato mb-4 text-center text-gray-800">Sei sicuro di voler eliminare {resourceToDelete.operationDate} ?</p>
...
export default FatturaAcquisto;
